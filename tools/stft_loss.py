...
def stft(x, fft_size, hop_size, win_length, window):
    ...
    Args:
        ...
        window (torch.Tensor): Window function type.
    Returns:
...
class STFTLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("window", getattr(torch, window)(win_length, device="cuda"))
...
class MultiResolutionSTFTLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        Args:
            fft_sizes (tuple): List of FFT sizes.
            hop_sizes (tuple): List of hop sizes.
            win_lengths (tuple): List of window lengths.
...
class L1MrstftLoss(torch.nn.Module):
    """Loss modules."""
    ...
    def __init__(self, cfg):
        super(L1MrstftLoss, self).__init__()
        self.ell_p_lamda = cfg["ell_p_lamda"]
        self.stft_lamda = cfg["stft_lamda"]
        self.STFTLoss = MultiResolutionSTFTLoss(**cfg["stft_config"])

    def forward(self, outputs, label):
        l1_loss = F.l1_loss(outputs, label)
        sc_loss, mag_loss = self.STFTLoss(outputs, label)
        loss = self.ell_p_lamda * l1_loss + (sc_loss + mag_loss) * self.stft_lamda
        return loss


class MseMrstftLoss(torch.nn.Module):
    def __init__(self, cfg):
        super(MseMrstftLoss, self).__init__()
        self.ell_p_lamda = cfg["ell_p_lamda"]
        self.stft_lamda = cfg["stft_lamda"]
        self.STFTLoss = MultiResolutionSTFTLoss(**cfg["stft_config"])

    def forward(self, outputs, label):
        l1_loss = F.mse_loss(outputs, label)
        sc_loss, mag_loss = self.STFTLoss(outputs, label)
        loss = self.ell_p_lamda * l1_loss + (sc_loss + mag_loss) * self.stft_lamda
        return loss
...
if __name__ == "__main__":
    cfg = toml.load(r"D:\Temp\config.toml")

    torch.manual_seed(1024)
    outputs = torch.rand([5, 320000])
    label = torch.rand([5, 320000]) * 0.1 + outputs

    loss_fun = L1MrstftLoss(cfg["loss"])
